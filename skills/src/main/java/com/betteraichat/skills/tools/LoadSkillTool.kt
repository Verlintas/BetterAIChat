...
class LoadSkillTool(private val skillProvider: () -> List<Skill>) : DeviceTool {
    ...
    override val description: String
        get() {
            ...
            val list = available.joinToString("；") { "${it.name}：${it.description}" }
            return "加载一个技能（skill）并严格按其指令执行任务。可用技能：$list"
        }
